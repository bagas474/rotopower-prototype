...
const STATUS_CFG: Record<WOStatus, { label: string; cls: string }> = {
  ...
  pending_planner: { label: "Pending Planner", cls: "bg-violet-100 text-violet-700 border border-violet-200" },
  planned:         { label: "Planned",         cls: "bg-blue-100 text-blue-700 border border-blue-200"      },
...
function CreateWOModal({ onSave, onClose }: { onSave: (wo: Omit<WorkOrder, "id" | "started_at" | "resolved_at">) => void; onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    title: "", description: "",
    priority: 2 as WOPriority, status: "draft" as WOStatus,
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={form.status} onChange={e => set("status", e.target.value as WOStatus)}
                  className="w-full px-3 py-2.5 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-400">
                  <option value="draft">Draft</option><option value="pending_planner">Pending Planner</option>
                </select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
