...
**Interaction Mapping (Frontend to Backend Schema):**

| Component Type | Frontend UI | Backend Target Schema | Meaning |
| :--- | :--- | :--- | :--- |
| **Logic Gate** | Dragged as an `AND` or `OR` block. | `expression` | Translated to `&` or `|` in the string. |
| **Sensor Node** | Dragged as a specific Tag block. | `sensor_codes` | Extracted from all sensor nodes on the canvas into an array. |
| **Fault Node** | Dragged as a cascading failure block. | `dependent_asset_fault_ids` | Extracted from all fault nodes into an array of integers. |
| **Connection Line** | Drawn between nodes. | `expression` | Defines parenthesis grouping `()` in the boolean string. |
...
**[Main Flow: Happy Path (Build & Save Tree - Update Transaction)]**
1. User arrives at the blank canvas for a specific RCA.
...
5. User clicks Save.
6. The Frontend compiles the visual nodes into three distinct payloads for the `PUT` request (`NewFaultTreeUpdate`):
   * `expression`: `(VIB_CODE & TEMP_CODE)`
   * `sensor_codes`: `["VIB_CODE", "TEMP_CODE"]`
   * `dependent_asset_fault_ids`: `[]` (if none are used)
