...
* **Cross-Module Integration (Work Orders):** Within the RCA view, there MUST be a "Linked Work Orders" panel or tab that lists all Work Orders currently attempting to resolve this specific `asset_fault_id`.
...
**[Main Flow: Manage Investigations (Create, Read, Delete)]**
1. **Create (C):** Engineer clicks "New Investigation" on the Data Grid. A modal opens requiring the user to select the `asset_fault_id` (the failure event) from a dropdown, and set the initial `priority` (1-5). Upon saving, a `NewFaultTree` record is created.
2. **Read (R):** System displays all `NewFaultTree` records in the Data Grid.
3. **Delete (D):** Engineer clicks the Trash icon on a row to delete the investigation entirely.
4. **Investigate (Update Flow Transition):** Engineer clicks "Investigate" (or a Canvas icon) on a specific row. The system routes them to the `Interactive_Tree` sub-page to build the actual logic (`expression`, `sensor_codes`).
